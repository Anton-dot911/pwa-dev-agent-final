// App.jsx — PWA Dev Agent Phase 1+3+4
// Chat · Streaming · Memory · ZIP · Design Engine
...
export default function App() {
  ...
  useEffect(() => {
    if (input.startsWith('/') && input.length > 0) {
      ...
      setSugg(COMMANDS.filter(c => c.startsWith(q) && c !== q.trim()))
    } else setSugg([])
  }, [input])
  ...
  const send = useCallback(async (overrideText) => {
    ...
    if (text === '/clear') { setMessages([WELCOME]); setExplorerData(null); return }

    const userMsg = { id: Date.now() + 'u', role: 'user', content: text }
    const apiMsgs = [...messages, userMsg].filter(m => m.id !== 'welcome').map(m => ({ role: m.role, content: m.content }))
    ...
    await streamChat({
      ...
      onDone: (total) => {
        ...
        if (files?.length > 0) {
          setExplorerData({ files, projectName })
          if (nameMatch) {
            addProject({ id: `proj-${Date.now()}`, name: nameMatch[1].slice(0, 40), stack: memory.stack, description: nameMatch[1], createdAt: new Date().toISOString().slice(0, 7) })
          }
        }
      },
      onError: (err) => {
        setStreamingId(null)
        setIsLoading(false)
        setMessages(prev => prev.map(m => m.id === aid ? { ...m, content: `❌ ${err}` } : m))
      },
    })
  ...
  const stats = explorerData ? getProjectStats(explorerData.files) : null
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh', background: B, fontFamily: "'DM Sans','Segoe UI',sans-serif", color: T }}>
      ...
      <div style={{ padding: '12px 16px', background: B, boxShadow: `0 4px 14px ${SD}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', zIndex: 10, gap: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: '#7a90a8' }}>Phase 4 · Design Engine · {memory.uiStyle}</div>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          {explorerData && (
            <button onClick={() => setExplorerData(d => ({ ...d }))} style={{ background: B, border: 'none', borderRadius: 10, padding: '7px 12px', fontSize: 12, fontWeight: '700', color: GR, boxShadow: neu(), cursor: 'pointer' }}>
              📦 {stats?.fileCount}f
            </button>
          )}
          <button onClick={() => setDesignOpen(true)} style={{ background: B, border: 'none', borderRadius: 10, padding: '7px 12px', fontSize: 12, fontWeight: '600', color: '#a06bc7', boxShadow: neu(), cursor: 'pointer' }}>🎨</button>
          <button onClick={() => setMemoryOpen(true)} style={{ background: B, border: 'none', borderRadius: 10, padding: '7px 12px', fontSize: 12, fontWeight: '600', color: AC, boxShadow: neu(), cursor: 'pointer' }}>🧠</button>
        </div>
      </div>

      {/* Messages */}
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 12px' }}>
        {messages.map((msg, i) => <MessageBubble key={msg.id || i} message={msg} isStreaming={msg.id === streamingId} />)}
        ...
        {explorerData && (
          <div style={{ margin: '12px 0', animation: 'fadeIn 0.3s ease' }}>
            <div style={{ background: B, boxShadow: neu(), borderRadius: 16, padding: '14px 18px', maxWidth: 400 }}>
              <div style={{ fontSize: 10, color: GR, fontWeight: '700', letterSpacing: '1px', textTransform: 'uppercase', marginBottom: 8 }}>📦 Готово до скачування</div>
              <div style={{ fontWeight: '700', fontSize: 14, color: T, marginBottom: 2 }}>{explorerData.projectName}</div>
              <div style={{ fontSize: 11, color: '#7a90a8', marginBottom: 12 }}>{stats?.fileCount} файлів · {stats?.totalLines} рядків · {stats?.totalKB} KB</div>
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => setExplorerData(d => ({ ...d }))} style={{ background: B, border: 'none', borderRadius: 9, padding: '7px 13px', fontSize: 12, fontWeight: '700', color: AC, boxShadow: neu(), cursor: 'pointer' }}>🗂 Файли</button>
                <button onClick={() => downloadProjectZip(explorerData.files, explorerData.projectName)} style={{ background: B, border: 'none', borderRadius: 9, padding: '7px 13px', fontSize: 12, fontWeight: '700', color: GR, boxShadow: neu(), cursor: 'pointer' }}>⬇ ZIP</button>
              </div>
            </div>
          </div>
        )}
      ...
      {suggestions.length > 0 && (
        <div style={{ padding: '0 12px 7px', display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {suggestions.map(s => (
            <button key={s} onClick={() => { setInput(s + ' '); inputRef.current?.focus() }} style={{ background: B, border: 'none', borderRadius: 8, padding: '4px 10px', fontSize: 12, color: AC, boxShadow: neu(), cursor: 'pointer', fontWeight: '600' }}>{s}</button>
          ))}
      ...
      {explorerData && <FileExplorer files={explorerData.files} projectName={explorerData.projectName} onClose={() => setExplorerData(null)} />}
